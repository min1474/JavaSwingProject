...
public class Booking extends JFrame {
	...
	public Booking() {
		...
		bookingButton.setBounds(466, 395, 188, 51);
		...
		JLabel noticeLabel = new JLabel("회원일 경우 마이페이지에서 예약 및 조회를 해주세요. 비회원은 이름과 연락처만 입력 후 예약 가능합니다");
		noticeLabel.setForeground(Color.GRAY);
		noticeLabel.setFont(new Font("나눔고딕 ExtraBold", Font.PLAIN, 11));
		noticeLabel.setBounds(408, 254, 505, 23);
		getContentPane().add(noticeLabel);
		
		JLabel noticeLabel2 = new JLabel("비회원 예약 조회는 이름과 연락처 날짜만 선택후 조회를 눌러주세요");
		noticeLabel2.setForeground(Color.GRAY);
		noticeLabel2.setFont(new Font("나눔고딕 ExtraBold", Font.PLAIN, 11));
		noticeLabel2.setBounds(408, 275, 505, 23);
		getContentPane().add(noticeLabel2);

		JButton searchBookingButton = new JButton("");
		searchBookingButton.setBackground(Color.WHITE);
		searchBookingButton.setIcon(new ImageIcon("./img/admin/searchforprint.png"));
		searchBookingButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (nameTextField.getText().equals("") || telTextField.getText().equals("")) {
					...
					// 인원 수랑 테이블 번호는 예약 조회하는데 필요 없어서 막아둠
					// dto.setPerson((String) peopleComboBox.getSelectedItem());
					// dto.setTableNum((String) tableBox.getSelectedItem());

					BookingDTO dtoB = dao.searchBookingNonMember(dto);

					if (nameTextField.getText().equals(dtoB.getName()) && telTextField.getText().equals(dtoB.getTel())
							&& yearComboBox.getSelectedItem().equals(dtoB.getYear())
							&& monthComboBox.getSelectedItem().equals(dtoB.getMonth())
							&& dayComboBox.getSelectedItem().equals(dtoB.getDay())
							&& timeComboBox.getSelectedItem().equals(dtoB.getHour())) {
						new SearchBookingInfoNonMember(dtoB);
					} else {
						JOptionPane.showMessageDialog(null, "입력하신 정보와 일치하는 예약 일정이 없습니다.");
					}
		...
		searchBookingButton.setBounds(669, 395, 188, 51);
		getContentPane().add(searchBookingButton);
		searchBookingButton.setBorderPainted(false);// 버튼 이미지 태두리와 배경 삭제
		searchBookingButton.setContentAreaFilled(true);
		searchBookingButton.setFocusPainted(false);
